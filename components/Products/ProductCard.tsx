...
function ProductCard() {
  ...
  const [selectedColor, setSelectedColor] = useState<string | null>(null); // State to store selected color
      ...
      <ClientHydration LoaderComponent={<OrderDetailsSkeleton />}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8 relative">
          ...
          <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col">
                ...
                {productData?.colors?.length ? (
                  <div className="flex items-center gap-2 mt-4">
                    <div className="text-gray-800 gap-2 flex text-lg">
                      {" "}
                      الالوان المتاحة:
                    </div>
                    <div className="flex gap-2">
                      {productData?.colors.map((color: any) => (
                        ...
                        >
                          <Button
                            ...
                            onClick={() => setSelectedColor(color?.name)} // Set selected color on click
                          >
                            {color?.stock}
                          </Button>

                          {/* <div>Stock: {color?.stock}</div> */}
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="md:hidden flex gap-4 sticky justify-between bottom-1 bg-white  p-2 rounded-lg z-40   shadow-xl">
              <div className="flex items-center w-[100px] h-12 justify-between border-[2px] gap-2">
                <Button
                  variant="light"
                  size="sm"
